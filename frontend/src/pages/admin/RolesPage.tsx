...
const PAGE_LABELS: Record<string, string> = {
  ...
  'retention-fields': 'Retention Fields',
};
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-5 space-y-4">
          <h3 className="text-sm font-semibold text-gray-700">{editId ? 'Edit Role' : 'New Role'}</h3>
          <div>
            ...
            <input
              ...
              className="w-full max-w-xs border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50"
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-2">Page Access</label>
            <div className="flex flex-wrap gap-3">
              {allPages.map((page) => (
                <label key={page} className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                ...
                {roles.map((r) => (
                  <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">
                      <div className="flex flex-wrap gap-1">
                        {r.permissions.length === 0 ? (
                          ...
                          r.permissions.map((p) => (
                            <span key={p} className="px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-700">
                              {PAGE_LABELS[p] ?? p}
                            </span>
                          ))
                        )}
                    ...
                    <td className="px-4 py-3 text-sm flex gap-2">
                      ...
                      {r.name === 'admin' && <span className="text-xs text-gray-400">Protected</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
