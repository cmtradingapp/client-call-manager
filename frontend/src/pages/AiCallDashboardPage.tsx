...
export function AiCallDashboardPage() {
  ...
  const th = 'px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap';
  ...
  return (
    <div className="space-y-4">
      {/* Period selector */}
      <div className="bg-white rounded-lg shadow px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className={thR + ' text-gray-500'} colSpan={2}>Unknown</th>
              ...
              <tr className="border-t border-gray-200">
                ...
                <th className={thR + ' text-gray-500'}>Calls</th>
                <th className={thR + ' text-gray-500'}>Mins</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-4 py-12 text-center text-sm text-gray-400">No data found.</td></tr>
              ) : (
                <>
                  {agents.map((a) => (
                    <tr key={a.agent_id} className="border-t border-gray-100 hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{a.agent_name}</td>
                      ...
                      <td className="px-4 py-3 text-sm text-right font-medium text-gray-500">{fmt(a.unknown_count)}</td>
                      <td className="px-4 py-3 text-sm text-right text-gray-500">{fmt(a.unknown_duration_mins)}</td>
                      <td className="px-4 py-3 text-sm text-right font-semibold text-gray-900">{fmt(a.total_calls)}</td>
                      <td className="px-4 py-3 text-sm text-right font-semibold text-gray-900">{fmt(a.total_duration_mins)}</td>
                    </tr>
                  ))}
                  <tr className="border-t-2 border-gray-300 bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-right font-bold text-gray-600">{fmt(totals.unknown_count)}</td>
                    <td className="px-4 py-3 text-sm text-right font-bold text-gray-600">{fmt(totals.unknown_duration_mins)}</td>
                    <td className="px-4 py-3 text-sm text-right font-bold text-gray-900">{fmt(totals.total_calls)}</td>
                    <td className="px-4 py-3 text-sm text-right font-bold text-gray-900">{fmt(totals.total_duration_mins)}</td>
                  </tr>
                </>
              )}
            </tbody>
