...
interface StatusData {
  ...
  positions_last: LastRecord | null;
...
export function ETLPage() {
  ...
  const sections = [
    ...
    { key: 'positions', label: 'Open Positions', source: 'dealio.positions', endpoint: '/etl/sync-positions', count: data?.positions_row_count ?? null, last: data?.positions_last ?? null, desc: 'Full refresh from dealio replica every 5 min (open positions change constantly). Used to compute Open PNL in the Retention Manager.' },
    ...
    { key: 'vtiger_users', label: 'Vtiger Users', source: 'report.vtiger_users', endpoint: '/etl/sync-vtiger-users', count: data?.vtiger_users_row_count ?? null, last: data?.vtiger_users_last ?? null, desc: 'Full refresh from MSSQL. Incremental sync runs every 30 min using modifiedtime (3h lookback).' },
    { key: 'vtiger_campaigns', label: 'Vtiger Campaigns', source: 'report.vtiger_campaigns', endpoint: '/etl/sync-vtiger-campaigns', count: data?.vtiger_campaigns_row_count ?? null, last: data?.vtiger_campaigns_last ?? null, desc: 'Full refresh from MSSQL. Incremental sync runs every 30 min using modifiedtime (3h lookback).' },
  ];
